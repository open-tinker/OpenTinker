#!/usr/bin/env python3
"""AndroidWorld RL Training Client.

This script trains an LLM agent to complete tasks in AndroidWorld.

Usage:
    # Start AndroidWorld server first (in another terminal):
    python -m opentinker.environment.android_world.android_world_server --port 8082

    # Run training:
    python android_world_rl.py scheduler_url=http://localhost:8780 num_gpus=2
"""
...
from opentinker.environment.android_world import AndroidWorldGame
...
@hydra.main(config_path="client_config", config_name="android_world_param.yaml")
def main(args):
    ...
    if enable_tracing:
        try:
            ...
            weave_project = args.get("weave_project", "android-world-training")
            init_weave_tracing(
    ...
    print("=" * 60)
    print("Training with AndroidWorld Environment")
    print("=" * 60)
    ...
    # 2. Setup GameEnvironment with AndroidWorldGame
    ...
    print("\nSetting up GameEnvironment with AndroidWorldGame...")
    ...
    if game_stats.health_check():
        print(f"✓ Connected to AndroidWorld server for metrics at {env_endpoint}")
        game_stats.reset_all()  # Reset all stats before training
    else:
        print(f"⚠ AndroidWorld server at {env_endpoint} not responding - metrics disabled")
        game_stats = None
